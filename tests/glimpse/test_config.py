...
class TestConfig:
    ...
    def test_init_invalid_destination(self, invalid_dest):
        """Test Config raises ValueError for invalid destinations."""
        with pytest.raises(ValueError, match=f"Invalid destination '{invalid_dest}'"):
            Config(dest=invalid_dest, env_override=False)

    @pytest.mark.parametrize("valid_dest", [
        "json", "jsonl", "sqllite", "mongo"
    ])
    def test_init_valid_destinations(self, valid_dest):
        """Test Config accepts all valid destinations."""
        with patch.dict(os.environ, {}, clear=True):
    ...
    def test_load_from_env_core_configs(self):
        """Test loading core configuration from environment variables."""
        env_vars = {
            "GLIMPSE_DEST": "json",
            "GLIMPSE_LEVEL": "debug",
    ...
    def test_load_from_env_trace_id_variations(self):
        ...
        for true_val in true_values:
            with patch.dict(os.environ, {"GLIMPSE_TRACE_ID": true_val}, clear=True):
                config = Config()
                assert config.enable_trace_id is True, f"'{true_val}' should be True"
        
        for false_val in false_values:
            with patch.dict(os.environ, {"GLIMPSE_TRACE_ID": false_val}, clear=True):
                config = Config()
    ...
    def test_load_from_env_partial_override(self):
        ...
        env_vars = {
            "GLIMPSE_DEST": "mongo",
            # LEVEL not provided - should use default
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            ...
            assert config.dest == "mongo"  # From environment
    ...
    def test_constructor_and_environment_integration(self):
        """Test integration between constructor parameters and environment variables."""
        env_vars = {
            "GLIMPSE_DEST": "mongo",  # Override constructor
    ...
    def test_environment_precedence_over_constructor(self):
        """Test that environment variables take precedence over constructor values."""
        env_vars = {
            "GLIMPSE_DEST": "mongo",
            "GLIMPSE_LEVEL": "error",
            "GLIMPSE_TRACE_ID": "1",
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            config = Config(
                dest="json",  # Should be overridden
    ...
    def test_empty_string_values(self):
        ...
        env_vars = {
            "GLIMPSE_DEST": "",  # Empty dest should still cause validation error
            "GLIMPSE_LEVEL": "",  # Empty level should be uppercased to ""
            "GLIMPSE_CUSTOM": "",  # Empty custom param should be preserved
        }
        
        with patch.dict(os.environ, env_vars, clear=True):
            # Empty dest should cause validation error
    ...
    def test_malformed_environment_variables(self):
        ...
        env_vars = {
            "GLIMPSE_DEST": "invalid_dest",  # Invalid destination
            "GLIMPSE_TRACE_ID": "maybe",  # Invalid boolean
        }
        
        with patch.dict(os.environ, env_vars, clear=True):
            # Invalid dest should cause error
            with pytest.raises(ValueError, match="Invalid destination: 'invalid_dest'"):
                Config()
    ...
    def test_config_creation_performance(self):
        """Test that Config creation is reasonably fast even with many env vars."""
        # Create many environment variables
        many_env_vars = {f"GLIMPSE_PARAM_{i}": f"value_{i}" for i in range(100)}
        many_env_vars["GLIMPSE_DEST"] = "jsonl"
    ...
    def test_isolated_environment_tests(self):
        """Test that environment mocking properly isolates tests."""
        # Test 1: Set specific environment
        with patch.dict(os.environ, {"GLIMPSE_DEST": "mongo"}, clear=True):
            config1 = Config()
            assert config1.dest == "mongo"
